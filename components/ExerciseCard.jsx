...
import { ChartIcon, InfoIcon, SparklesIcon } from './Icons.jsx';

const ExerciseCard = ({ exercise, logs, onLogChange, currentDate }) => {
  ...
  const handleGetAiTip = useCallback(async () => {
    ...
    try {
        // Lazy load the AI library only when needed
        ...
        if (e.message === 'API key not configured') {
            setError('Funcionalidad de IA no configurada. Añade tu API key de Gemini.');
        } else {
